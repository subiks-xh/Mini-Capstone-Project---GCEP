...
async function testCategoryManagement() {
  ...
  const singleResponse = await axios.get(
    `${BASE_URL}/api/admin/categories/${testCategoryId}`,
    {
      headers: { Authorization: `Bearer ${adminToken}` },
    }
  );
...
async function testComplaintManagement() {
  ...
  const complaintData = {
    ...
    description:
      "This is a test complaint about network connectivity issues in the office.",
  ...
  console.log(
    `   - Found ${getUserComplaints.data.complaints.length} complaints for user`
  );
  ...
  console.log(
    `   - Found ${getStaffComplaints.data.complaints.length} complaints for staff`
  );

  // Get single complaint
  const singleResponse = await axios.get(
    `${BASE_URL}/api/complaints/${testComplaintId}`,
    {
      headers: { Authorization: `Bearer ${userToken}` },
    }
  );
  ...
  if (staffToken && adminToken) {
    try {
      ...
      await axios.patch(
        `${BASE_URL}/api/complaints/${testComplaintId}/assign`,
        {
    ...
    try {
      await axios.patch(
        `${BASE_URL}/api/complaints/${testComplaintId}/status`,
        {
          status: "in-progress",
...
async function testAnalytics() {
  ...
  const analyticsResponse = await axios.get(
    `${BASE_URL}/api/complaints/analytics`,
    {
      headers: { Authorization: `Bearer ${staffToken}` },
    }
  );
  ...
  console.log(
    `   - Total complaints: ${analyticsResponse.data.analytics.general.totalComplaints}`
  );
}
...
async function testAdminDashboard() {
  ...
  console.log(
    `   - Total users: ${dashboardResponse.data.dashboard.totals.users}`
  );
  console.log(
    `   - Total complaints: ${dashboardResponse.data.dashboard.totals.complaints}`
  );
