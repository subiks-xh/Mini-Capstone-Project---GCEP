...
const { COMPLAINT_STATUS, COMPLAINT_PRIORITY, USER_ROLES } = require('../config/constants');
  ...
  static async getOverviewAnalytics(dateRange = {}) {
    try {
      ...
      const overdueComplaints = await Complaint.countDocuments({
        ...
        status: { 
          $nin: [COMPLAINT_STATUS.RESOLVED, COMPLAINT_STATUS.CLOSED, COMPLAINT_STATUS.ESCALATED] 
        }
      });
      ...
      return {
        overview: {
          totalComplaints: statusStats.reduce((sum, stat) => sum + stat.count, 0),
          activeComplaints,
          resolvedComplaints: statusBreakdown[COMPLAINT_STATUS.RESOLVED] || 0,
          overdueComplaints,
          escalatedComplaints,
          avgResolutionTimeHours: resolutionStats[0]?.avgResolutionTime?.toFixed(2) || 0
        },
        breakdown: {
        ...
        performance: {
          resolutionRate: statusStats.reduce((sum, stat) => sum + stat.count, 0) > 0
            ? ((statusBreakdown[COMPLAINT_STATUS.RESOLVED] || 0) / statusStats.reduce((sum, stat) => sum + stat.count, 0) * 100).toFixed(2)
            : 0,
          escalationRate: statusStats.reduce((sum, stat) => sum + stat.count, 0) > 0
            ? (escalatedComplaints / statusStats.reduce((sum, stat) => sum + stat.count, 0) * 100).toFixed(2)
            : 0
        }
      };
    ...
    try {
      ...
      return {
        ...
        summary: {
          ...
          totalSubmitted: formattedTrend.reduce((sum, item) => sum + item.submitted, 0),
          totalResolved: formattedTrend.reduce((sum, item) => sum + item.resolved, 0),
          totalEscalated: formattedTrend.reduce((sum, item) => sum + item.escalated, 0)
        }
      };
    ...
    try {
        ...
        },
        {
          $addFields: {
            ...
            escalationRate: {
              $cond: [
                { $gt: ['$totalComplaints', 0] },
                { $multiply: [{ $divide: ['$escalated', '$totalComplaints'] }, 100] },
                0
              ]
            }
          }
      ...
      const processedCategoryStats = categoryStats.map(cat => {
        ...
        return {
          ...
          avgResolutionTime: cat.avgResolutionTime ? cat.avgResolutionTime.toFixed(2) : 0,
      ...
      return {
        ...
        summary: {
          ...
          avgResolutionRate: processedCategoryStats.length > 0
            ? (processedCategoryStats.reduce((sum, cat) => sum + parseFloat(cat.resolutionRate), 0) / processedCategoryStats.length).toFixed(2)
            : 0
        }
      };
  ...
  static async getStaffAnalytics(dateRange = {}) {
    try {
      ...
      const processedStaffStats = staffStats.map(staff => ({
        ...staff,
        avgResolutionTime: staff.avgResolutionTime ? staff.avgResolutionTime.toFixed(2) : 0,
      ...
      return {
        ...
        summary: {
          ...
          avgResolutionRate: processedStaffStats.length > 0
            ? (processedStaffStats.reduce((sum, staff) => sum + parseFloat(staff.resolutionRate), 0) / processedStaffStats.length).toFixed(2)
            : 0,
          topPerformer: processedStaffStats.length > 0
            ? processedStaffStats.reduce((top, staff) => parseFloat(staff.resolutionRate) > parseFloat(top.resolutionRate) ? staff : top)
            : null
        }
      };
    ...
    try {
      ...
      return {
        summary: {
          ...
          avgOverallRating: stats.avgOverallRating ? stats.avgOverallRating.toFixed(2) : 0,
          avgResponseTimeRating: stats.avgResponseTimeRating ? stats.avgResponseTimeRating.toFixed(2) : 0,
          avgResolutionQualityRating: stats.avgResolutionQualityRating ? stats.avgResolutionQualityRating.toFixed(2) : 0,
          avgCommunicationRating: stats.avgCommunicationRating ? stats.avgCommunicationRating.toFixed(2) : 0
        },
        ratingDistribution: ratingDistribution.reduce((acc, item) => {
          acc[`${item._id}star`] = item.count;
          return acc;
        }, {}),
        satisfactionLevel: this.calculateSatisfactionLevel(stats.avgOverallRating || 0)
      };
  ...
  static async getSLAPerformance(dateRange = {}) {
    try {
        ...
        },
        {
          $addFields: {
            slaCompliance: {
              $cond: [
                { $gt: ['$totalComplaints', 0] },
                { $multiply: [{ $divide: ['$resolvedOnTimeCount', '$totalComplaints'] }, 100] },
                0
              ]
            }
          }
      ...
      const processedSLAStats = slaStats.map(stat => ({
        ...
        avgTimeToResolution: stat.avgTimeToResolution ? stat.avgTimeToResolution.toFixed(2) : 0
      }));

      // Calculate overall SLA compliance
      const overallCompliance = slaStats.length > 0
        ? (slaStats.reduce((sum, stat) => sum + stat.slaCompliance, 0) / slaStats.length).toFixed(2)
        : 0;
      ...
      return {
        ...
        overall: {
          ...
          totalComplaints: slaStats.reduce((sum, stat) => sum + stat.totalComplaints, 0),
          totalOverdue: slaStats.reduce((sum, stat) => sum + stat.overdueCount, 0),
          totalResolvedOnTime: slaStats.reduce((sum, stat) => sum + stat.resolvedOnTimeCount, 0)
        }
      };
