...
const feedbackSchema = new mongoose.Schema({
  ...
  rating: {
    ...
    min: [FEEDBACK_RATING.MIN, `Rating must be at least ${FEEDBACK_RATING.MIN}`],
  ...
  aspects: {
    resolution: {
      rating: {
        type: Number,
        min: [FEEDBACK_RATING.MIN, `Resolution rating must be at least ${FEEDBACK_RATING.MIN}`],
        max: [FEEDBACK_RATING.MAX, `Resolution rating cannot exceed ${FEEDBACK_RATING.MAX}`]
      },
      comment: {
        type: String,
        trim: true,
    ...
    timeliness: {
      rating: {
        type: Number,
        min: [FEEDBACK_RATING.MIN, `Timeliness rating must be at least ${FEEDBACK_RATING.MIN}`],
        max: [FEEDBACK_RATING.MAX, `Timeliness rating cannot exceed ${FEEDBACK_RATING.MAX}`]
      },
      comment: {
        type: String,
        trim: true,
    ...
    communication: {
      rating: {
        type: Number,
        min: [FEEDBACK_RATING.MIN, `Communication rating must be at least ${FEEDBACK_RATING.MIN}`],
        max: [FEEDBACK_RATING.MAX, `Communication rating cannot exceed ${FEEDBACK_RATING.MAX}`]
      },
      comment: {
        type: String,
        trim: true,
        maxlength: [500, 'Communication comment cannot exceed 500 characters']
      }
    },
    ...
    professionalism: {
      rating: {
        type: Number,
        min: [FEEDBACK_RATING.MIN, `Professionalism rating must be at least ${FEEDBACK_RATING.MIN}`],
        max: [FEEDBACK_RATING.MAX, `Professionalism rating cannot exceed ${FEEDBACK_RATING.MAX}`]
      },
      comment: {
        type: String,
        trim: true,
        maxlength: [500, 'Professionalism comment cannot exceed 500 characters']
      }
    }
  },
...
feedbackSchema.pre('save', function(next) {
  if (this.isNew) {
    ...
    if (!this.rating && this.aspects) {
      ...
      if (this.aspects.resolution?.rating) aspectRatings.push(this.aspects.resolution.rating);
      if (this.aspects.timeliness?.rating) aspectRatings.push(this.aspects.timeliness.rating);
      if (this.aspects.communication?.rating) aspectRatings.push(this.aspects.communication.rating);
      if (this.aspects.professionalism?.rating) aspectRatings.push(this.aspects.professionalism.rating);
      
      if (aspectRatings.length > 0) {
        this.rating = Math.round(aspectRatings.reduce((a, b) => a + b, 0) / aspectRatings.length);
      }
    }
...
feedbackSchema.virtual('aspectsAverage').get(function() {
  ...
  const ratings = [];
  if (this.aspects.resolution?.rating) ratings.push(this.aspects.resolution.rating);
  if (this.aspects.timeliness?.rating) ratings.push(this.aspects.timeliness.rating);
  if (this.aspects.communication?.rating) ratings.push(this.aspects.communication.rating);
  if (this.aspects.professionalism?.rating) ratings.push(this.aspects.professionalism.rating);
