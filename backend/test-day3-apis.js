...
const testComplaint = {
  ...
  description:
    "Critical system outage affecting all users. This is a test complaint for escalation functionality.",
...
async function testAuthentication() {
  ...
  // Register staff user
  ({ response, result } = await makeRequest(
    "POST",
    "/auth/register",
    staffUser
  ));
...
async function testStaffManagement() {
  ...
  if (testCategoryId) {
    await makeRequest(
      "GET",
      `/staff/available/${testCategoryId}`,
      null,
      adminToken
    );
  }
...
async function testComplaintAssignment() {
  ...
  // Get staff ID
  const { response: meResponse, result: meResult } = await makeRequest(
    "GET",
    "/auth/me",
    null,
    staffToken
  ...
  const { response: createResponse, result: createResult } = await makeRequest(
    "POST",
    "/complaints",
    autoAssignComplaint,
    adminToken
...
async function testEscalationManagement() {
  ...
  await makeRequest(
    "GET",
    "/admin/escalations/at-risk?hours=24",
    null,
    adminToken
  );
  ...
  await makeRequest(
    "GET",
    `/admin/escalations/stats?startDate=${thirtyDaysAgo}&endDate=${today}`,
    null,
    adminToken
  );
...
async function testBackgroundJobs() {
  ...
  console.log(
    "⚠️  Skipping escalation interval update to avoid system disruption"
  );
}
...
async function testAnalyticsReporting() {
  ...
  await makeRequest(
    "GET",
    `/admin/analytics/overview?startDate=${thirtyDaysAgo}&endDate=${today}`,
    null,
    adminToken
  );
  ...
  await makeRequest(
    "GET",
    `/admin/analytics/report?startDate=${thirtyDaysAgo}&endDate=${today}&trendDays=30`,
    null,
    adminToken
  );
  ...
  console.log(
    "⚠️  Note: Some detailed analytics endpoints might not be implemented in routes yet"
  );
}
...
async function testSystemIntegration() {
  ...
  const urgentComplaint = {
    ...
    description:
      "This complaint is designed to test the full escalation pipeline",
    category: testCategoryId,
    priority: "urgent",
    contactMethod: "phone",
  };
  ...
  if (response && response.ok && result.success) {
    ...
    console.log(
      "✅ Urgent complaint created for integration test:",
      urgentComplaintId
    );
    ...
    await makeRequest(
      "GET",
      "/admin/escalations/at-risk?hours=1",
      null,
      adminToken
    );
  }
...
async function runAllTests() {
  ...
  try {
    ...
    if (!dataSuccess) {
      console.log(
        "❌ Test data setup failed, continuing with limited tests..."
      );
    }
